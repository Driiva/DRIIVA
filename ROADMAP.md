...
- [ ] Dashboard breaks three design laws, invisible until the gate could reach it: capsule radii on five painted oblongs (the indigo badge and both progress bars), body copy at 13px against a 15px floor on three strings plus "Trust Centre" at 11px against a 13px floor, and five non-tabular numeric readouts
- [ ] Leaderboard breaks the tabular-figures law on its five SVG chart axis labels (0, 7, 14, 21, 28)
...
- [x] Resume threshold labelling mismatch (review finding 8). Resuming from PAUSED needed 4.5 m/s while the stationary clock cleared at 1.0 m/s, so through most of a stop-start urban crawl the car was moving, was not accumulating toward the end of the trip, and the screen still read "Stopped. Still recording." Both directions now come off one line, `MOVING_SPEED_MPS` (1.0), which is the line the stationary clock already used, so the label and the countdown to ending cannot disagree again. No separate hysteresis band was needed: `PAUSE_HOLD_MS` already is one, since returning to paused takes a full minute with nothing above the line. The high bar stays where it belongs, on STARTING a drive from nothing, where a walk must never open a trip - *done: the resume/stationary-clock invariant is now a property test rather than two constants that happened to agree; 1119 passing, 8 of 8 mobile source laws and 8 of 8 fabrication laws green*
