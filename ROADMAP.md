...
- [x] Resume threshold labelling mismatch (review finding 8). Resuming from PAUSED needed 4.5 m/s while the stationary clock clears at 1.0 m/s, so between the two the screen could say "Stopped. Still recording." while the car crawled. It recorded correctly throughout, so this was labelling rather than lost data - *raised by `e7febe2`, fixed by `db9cc03`: `fromPaused` now resumes at `PAUSE_SPEED_MPS`, the same threshold that already clears the stationary clock, instead of the higher `START_SPEED_MPS` bar reserved for declaring a brand new drive*
