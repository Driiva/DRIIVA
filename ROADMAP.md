...
- [x] Split `server/routes.ts` into domain-specific route modules - *done: `nightly/2026-09-07`. The 1,519-line `registerRoutes` is now `server/routes/index.ts` mounting seven domain modules (`auth`, `telematics`, `community`, `gdpr`, `ai`, `payments`, `webhooks`), with the leaderboard cache and the Stripe payment-succeeded glue lifted into `server/lib`. Code moved, not rewritten: same 36 registrations, same middleware chains. Held by `server/__tests__/route-inventory.test.ts` (method, path and handler count pinned for every route, proven red when `requireAuth` is dropped from one) and `tests/unit/server-route-modules.test.ts` (monolith stays gone, 500-line ceiling, every module mounted). Full suite 1172 passing, tsc error set unchanged, build green.*
