...
- [ ] `npm run gates` no longer reports INCOMPLETE - it reaches every route and now reports real violations instead. Sign-in was being refused by our own Content-Security-Policy: `connect-src 'self'` against a page on `localhost:5202` and an Auth emulator on `127.0.0.1:9098`, which Firebase surfaces as `auth/network-request-failed`, indistinguishable from a real network fault. That is why extensions, ports and env files each looked like the answer and none of them were. Coverage went from 1 of 5 design-law routes and 7 of 14 axe routes to 5 of 5 and 14 of 14. The ticket stays open because a run is still not green, but on the three findings below rather than on reach - *advanced by `316781c`, `299b131`, `69b41cf`, and the CSP fix*
- [x] Dashboard breaks three design laws, invisible until the gate could reach it: capsule radii on five painted oblongs (the indigo badge and both progress bars), body copy at 13px against a 15px floor on three strings plus "Trust Centre" at 11px against a 13px floor, and five non-tabular numeric readouts - *done at source: `nightly/2026-09-01`. All five capsules (Beta badge, both progress bars' track and fill) plus the loading-skeleton twin re-radiused per the shimmer precedent (2px literal on thin bars, `--radius-xs` on the badge - `rounded-sm` was avoided because its `calc(var(--radius) - 4px)` references an undefined var and computes to 0); footer row 11px to 13px; the starting-score explainer's 330-char body copy 13px to 15px; seven plain numeric spans given `.tabular`. Pinned in CI by `tests/unit/web-dashboard-laws.test.ts` with planted-violation proofs. The browser gate itself still cannot run unattended, so the parent gates ticket stays open; the page has also drifted since the 22 Aug gate run and carries further 12px long-copy law-5 debt the next real gate run will enumerate*
- [ ] Leaderboard breaks the tabular-figures law on its five SVG chart axis labels (0, 7, 14, 21, 28) - *implemented on open PR #68 (`nightly/2026-08-28`), awaiting review - do not re-implement*
- [ ] Rewards has five SERIOUS axe colour-contrast nodes, all of them the `opacity-50` locked reward cards - the dimming that signals "locked" is what takes the text under threshold
- [x] Browser Sentry reports never leave the page. `connect-src` has no `https://*.ingest.sentry.io`, in production as well as dev, so every envelope POST is refused by CSP. Observed as a live `securitypolicyviolation` while debugging the gate. "Complete Sentry wiring" is ticked in the Observation Mode sprint and the client half of it has never delivered an event - *done on main: `b6fe697`, 25 Aug; this checkbox was stale, caught 1 Sep*
...
- [ ] Resume threshold labelling mismatch (review finding 8). Resuming from PAUSED needs 4.5 m/s while the stationary clock clears at 1.0 m/s, so between the two the screen can say "Stopped. Still recording." while the car crawls. It records correctly throughout, so this is labelling rather than lost data - *raised by `e7febe2`. IMPLEMENTED FOUR TIMES on open, unmerged nightly PRs: #67 (26 Aug), #70 (29 Aug), #71 (30 Aug), #83 (31 Aug). Do not re-implement; the blocker is PR review, not code. Merge one, close the rest*
