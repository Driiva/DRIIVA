...
- [ ] **Mobile background trip capture** - on `feat/mobile-background-trip-capture` (not yet merged to main): `expo-task-manager` + `Location.startLocationUpdatesAsync` now feed the same `TripPointWriter` the foreground watch already uses, gated behind an explicit in-app ask for "Always" location before the OS prompt appears. `tsc` clean, `expo config` parses, pure buffering logic unit tested (`tests/unit/mobile-background-location.test.ts`). Unverified: all real background execution on a physical device - no device or simulator available in the authoring environment. Existing installed binaries need a fresh native (EAS) build before this JS can run at all.
