...
- [x] Resume threshold labelling mismatch (review finding 8). Resuming from PAUSED needed 4.5 m/s while the stationary clock cleared at 1.0 m/s, so between the two the screen could say "Stopped. Still recording." while the car crawled - *done: `5810e51`, one threshold (`PAUSE_SPEED_MPS`) now decides stationary versus moving in both directions; no speed hysteresis band needed because resuming is instant while pausing again takes the full 60s hold, which is pinned by test. Trip lifetimes untouched - the monitor ignores pause/resume events and ending runs off the shared movement clock in either state*
