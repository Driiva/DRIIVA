...
const BOX = { width: 320, height: 190, pad: 18 };
 ...
 * long enough to stall a bridge that has to serialise it. The shape of a route
 * survives thinning; the bridge does not survive not thinning.
 */
...
/** Lat and lng to the drawing box, aspect preserved, centred, padded. */
function project(points: TracePoint[]): Projected | null {
  ...
  // the equator. Without this the same drive looks stretched sideways in
  // Sheffield and almost square in Quito.
...
/** Every Nth sample, with the last point always kept so the trace ends where the trip did. */
function thin<T>(items: T[], max: number): { items: T[]; step: number } {
...
export function RouteTrace({ points, markers = [], duration = 1400 }: RouteTraceProps) {
  const { items: sampled, step } = useMemo(() => thin(points, MAX_POINTS), [points]);
  const projected = useMemo(() => project(sampled), [sampled]);
