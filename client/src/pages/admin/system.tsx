              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.08]">
                    ...
                    <th className="text-left px-4 py-3 text-white/60 font-medium text-xs uppercase tracking-wider">Last Sync</th>
                    <th className="text-left px-4 py-3 text-white/60 font-medium text-xs uppercase tracking-wider">Successes</th>
                    <th className="text-left px-4 py-3 text-white/60 font-medium text-xs uppercase tracking-wider">Failures</th>
                ...
                <tbody>
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-white/60 whitespace-nowrap">{formatTimestamp(log.timestamp)}</td>
                      <td className="px-4 py-3 text-emerald-400 tabular-nums">{log.successCount}</td>
                      <td className={`px-4 py-3 tabular-nums ${log.hasFailures ? 'text-amber-400 font-medium' : 'text-white/60'}`}>
                        {log.failureCount}
                      </td>
                      <td className="px-4 py-3">
                        {log.timestamp === null && log.successCount === 0 ? (
                          <span className="text-white/55 text-xs">No data</span>
                        ) : log.hasFailures ? (
