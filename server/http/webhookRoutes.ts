/**
 * Webhook routes: Stripe and Root Platform. Extracted verbatim from
 * server/routes.ts. Both take a raw body (server/app.ts registers express.raw
 * for these paths) because both verify a signature over the exact bytes.
 */
...
import { handleStripePaymentSucceeded, resolveSubscriptionBillingPeriod } from "./stripeIntegration";

/**
 * The invoice fields these handlers read. Declared structurally rather than as
 * Stripe.Invoice because the pinned API version (2025-01-27.acacia, see
 * server/lib/stripe.ts) still carries `subscription` at the invoice root,
 * which stripe-node's current types no longer describe.
 */
...
/** The subscription fields the customer.subscription.deleted branch reads. */
interface WebhookSubscription {
  id: string;
  customer?: string | null;
}

/** The session fields the checkout.session.completed branch reads. */
...
export function registerWebhookRoutes(app: Express): void {
  ...
  app.post("/api/webhooks/stripe", webhookLimiter, async (req, res) => {
    ...
    try {
      ...
      parsedEvent = stripe.webhooks.constructEvent(req.body as Buffer, sig, webhookSecret);
    } catch (err) {
      const reason = err instanceof Error ? err.message : String(err);
      console.error("[Stripe webhook] Signature verification failed:", reason);
      return res.status(400).json({ message: `Webhook Error: ${reason}` });
    }
    // Past the verification guard the event is always present: the catch above
    // returns. Binding it to a const is what lets the rest of this handler be
    // typed without the `any` it used to carry.
    ...
    try {
      switch (event.type) {
        ...
        case 'checkout.session.completed': {
          const session = event.data.object as unknown as WebhookCheckoutSession;
