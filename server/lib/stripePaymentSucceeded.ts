/**
 * Stripe -> policy integration glue, called from the Stripe webhook handler
 * (server/routes/webhooks.ts) on invoice.payment_succeeded.
 *
 * Binds or re-activates the Postgres policy through the lifecycle state
 * machine, emits the pool-contribution seam, and mirrors a pendingPayment
 * into Firestore. Every critical step rethrows so the webhook 500s and Stripe
 * redelivers; retries are safe because the bind is idempotent per subscription.
 */
import { storage } from "../storage";
import type { Policy } from "@shared/schema";
import { transitionPolicy, createPolicyWithAudit, InvalidPolicyTransitionError } from "./policyLifecycle";
import { emitPoolContribution } from "./poolContribution";
 ...
 * (create-subscription in routes/payments.ts always sets it), falling back to
 ...
 */
export function resolveSubscriptionBillingPeriod(sub: any): 'monthly' | 'annual' {
