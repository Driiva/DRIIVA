/**
 * Telematics routes: the driver's own dashboard, trips, aggregated scores,
 * incident reports and rule-based insights.
 *
 * Every route here carries :userId and is guarded by requireResourceOwner, so
 * the userId used for the lookup is the one resolved from the verified token,
 * never the one in the URL.
 */
...
export function registerTelematicsRoutes(app: Express): void {
