/**
 * Auth routes: the signed-in profile, Firebase ID token exchange, and the
 * WebAuthn (Face ID / Touch ID) passkey endpoints.
 *
 * Passkey lookups use email, not username. Enrolment is an authenticated
 * action bound to the verified token's email; authentication is public and
 * returns a Firebase custom token for the client to sign in with.
 */
